...
function checkCollisions(sheets, kerf = 3) {
  let collisions = 0;
...
console.log("=== Testing Sheet Compaction on Benchmark Result ===");
res.sheets.forEach(s => {
  const beforeMaxX = Math.max(...s.placed.map(p => p.x + p.w));
  const compacted = gravityCompactSheet(s, config);
  const afterMaxX = Math.max(...compacted.placed.map(p => p.x + p.w));
  const collisions = checkCollisions([compacted]);
  console.log(`Sheet ${s.id}:`);
  console.log(`  Before Max X: ${beforeMaxX} mm (Remnant: ${s.sheetLength - beforeMaxX} mm)`);
  console.log(`  After Max X:  ${afterMaxX} mm (Remnant: ${s.sheetLength - afterMaxX} mm)`);
